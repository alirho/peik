...
-   **مسئولیت‌ها**:
    -   **`storageService.js`**: انتزاعی‌سازی عملیات خواندن و نوشتن در `localStorage`. این سرویس اکنون به جای یک تاریخچه، کل آرایه چت‌ها را مدیریت می‌کند.
...
-   **ذخیره‌سازی سمت کلاینت**: برای سادگی و عدم نیاز به بک‌اند، تمام داده‌ها در `localStorage` ذخیره می‌شوند. این تصمیم باعث می‌شود برنامه کاملاً ایستا باشد.
